<script lang="ts">
	...
	let depthClampingActiveUniform: WebGLUniformLocation;
	...
	async function initializeProgram() {
		const shaderList = await Promise.all([
			loadShader(gl.VERTEX_SHADER, 'depth-clamping.vert', 5),
			loadShader(gl.FRAGMENT_SHADER, 'depth-clamping.frag', 5)
		]);
		...
		const depthClampingUniformLocation = gl.getUniformLocation(theProgram, 'depthClampingActive');
		assert(depthClampingUniformLocation);
		depthClampingActiveUniform = depthClampingUniformLocation;
		...
		gl.uniform1i(depthClampingActiveUniform, 0);
		gl.useProgram(null);
	}
	...
	function display() {
		...
		gl.uniform3f(offsetUniform, 0, 0, 0.5);
		gl.drawElements(gl.TRIANGLES, indexData.length / 2, gl.UNSIGNED_SHORT, 0);

		gl.uniform3f(offsetUniform, 0, 0, -1);
		// this should instead use the equivalent of glDrawElementsBaseVertex, but
		// as it's not available in WebGL, the indexData is extended to cover both objects
		gl.drawElements(gl.TRIANGLES, indexData.length / 2, gl.UNSIGNED_SHORT, indexData.length / 2);
	...
	function keyboard(key: number) {
		switch (key) {
			case 27:
				...
				break;
			case 32:
				// use a boolean uniform and perform the clamping manually in the frag
				// shader, passing through the non-clipped Z coordinate in a varying
				depthClampingActive = !depthClampingActive;
				gl.useProgram(theProgram);
				gl.uniform1i(depthClampingActiveUniform, depthClampingActive ? 1 : 0);
				gl.useProgram(null);
		}
	}
</script>
